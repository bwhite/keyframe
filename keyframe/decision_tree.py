...
class DecisionTree(keyframe.BaseKeyframer):
    ...
    def train(self, gt_path, video_path, hard_negatives=False):
        ...
        for vid_num, gt_fn in enumerate(glob.glob(gt_path + '/*.txt')):
            ...
            if not hard_negatives:
                cur_num_neg = min(len(neg_diffs), 10)
        ...
        neg_label_values = random.sample(neg_label_values, num_samples)
    ...
    def train2(self):
        ...
        values = np.vstack(zip(*label_values)[1])
        dims = zip(np.min(values, 0), np.max(values, 0))
        print('Len dims: %s' % len(dims))
        print('Shape values: %s' % (values.shape,))
        self.rfc = classipy.RandomForestClassifier(classipy.rand_forest.VectorFeatureFactory(dims, np.zeros(len(dims)), 10), num_feat=100)
        self.rfc.train(label_values)
    ...
    def _hist_feat_func(self, frame):
        ...
        out = []
        for block, trans in imfeat.BlockGenerator(frame, cgr, output_size=(height / 2, width / 2), step_delta=(height / 2, width / 2)):
    ...
    def _diff_func(self, points0, points1):
        ...
        return np.hstack([num_match, hist_diff])
